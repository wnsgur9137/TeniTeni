...
⏸ **보류 확정** — [D-20](../05-결정/stack/D-20-server-ml.md). Phase 2를 **CPU 추론으로 시작**하고, 고정 샘플 10개에 대한 온디바이스 대비 정량 비교 후 도입 여부를 결정합니다. 개선폭이 작으면 서버 ML을 도입하지 않고 서버는 저장·집계·리포트만 담당합니다.
