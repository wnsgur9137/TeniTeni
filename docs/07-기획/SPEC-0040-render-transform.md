...
| 코드가 내는 `sx` | 1.778/0.462 = **3.847** |
| 회전 후 실제 버퍼 | 1080×1920 → 종횡비 **0.5625** |
| 올바른 `sx` | 0.5625/0.462 = **1.217** |

**스케일이 3.16배 틀립니다.** 회전을 적용하면 비균등 스케일이 축과 함께 돌아가므로, 회전 전 종횡비로 계산한 값을 그대로 쓸 수 없습니다.

두 경우의 NDC 범위를 실제로 계산하면 이렇습니다.

| | x 범위 | y 범위 |
|---|---|---|
| 4.3 코드 | [-1.000, 1.000] | **[-3.847, 3.847]** |
| 정정 | [-1.000, 1.000] | [-1.217, 1.217] |

**틀린 것은 "어느 축이 넘치는가"가 아니라 "얼마나 넘치는가"입니다.** `sx`는 회전 전 x축에 곱해지고 90° 회전으로 화면의 **세로축**이 되므로, 넘치는 방향 자체는 두 경우가 같습니다. 다만 4.3은 필요한 1.217배 대신 **3.847배로 확대**합니다 — 화면에 보이는 것이 실제 화각의 3분의 1이 됩니다.

관절 좌표에도 같은 행렬이 곱해지므로 **스켈레톤은 영상과 어긋나지 않습니다.** 둘 다 똑같이 과확대됩니다. 그래서 이 결함은 "스켈레톤이 안 맞는다"로 드러나지 않고 **"화면이 너무 확대돼 보인다"** 로 드러납니다 — 실기기 없이는 눈치채기 어렵고, 그래서 지금 테스트로 고정합니다.
...
- [ ] **90° 회전에서 스케일이 회전 후 종횡비로 계산된다** — 세로축 NDC 범위가 `[-3.847, 3.847]`이 아니라 `[-1.217, 1.217]` (선택지 2)
- [ ] **뷰를 꽉 채우되 필요 이상 확대하지 않는다** — 한 축이 정확히 `[-1, 1]`을 덮고 다른 축만 넘친다
