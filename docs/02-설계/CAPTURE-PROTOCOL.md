...
### 관찰 대상이 라인 판정이면 가이드가 다르다

5.7의 배치가 5.1과 다르므로 **같은 가이드를 쓸 수 없습니다** ([ADR-0004](../05-결정/adr/ADR-0004-line-call-mode.md)).

| 요소 | 내 스윙 | **라인 판정** |
|---|---|---|
| 거리 가이드 | 인체 실루엣, 프레임 높이 35~45% | **라인 전체가 들어오는지** |
| 각도 안내 | "코트 옆에서 옆모습으로" | **"판정할 라인의 연장선에"** |
| 추가 단계 | 없음 | **4점 탭 → 코트 격자 검증** |
| 수평·흔들림·조명 | 동일 | 동일 |

**격자 검증이 라인 판정의 프레이밍 가이드입니다.** 코트 격자를 영상에 겹쳐 보여주면 배치가 맞는지, 캘리브레이션이 맞는지가 **한 화면에서 동시에** 드러납니다.

흔들림이 심하면 **조용히 실패하지 않고 이유를 보여주는 것**이 중요합니다. `DetectTrajectoriesRequest`는 고정 카메라가 전제이며, 이건 튜닝으로 해결되지 않습니다.

## 5.7 관찰 대상이 라인 판정일 때의 배치

> 관찰 대상이 "내 스윙"일 때의 배치(5.1)와 **같이 쓸 수 없습니다.** 근거: [ADR-0004](../05-결정/adr/ADR-0004-line-call-mode.md)
...
- ⬜ **어느 쪽 측면에서 찍는가** — 5.1이 "측면 6m"까지만 정했다. 주 사용 손에 따라 보이는 몸이 다르다 ([IA-FLOW 10.10](../06-디자인/IA-FLOW.md))
- ⬜ **라인 판정의 정확도** — 검증 기준이 없다. 클레이 바운스 자국 실측이 필요하다 (5.7)
