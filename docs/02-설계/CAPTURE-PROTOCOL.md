...
> ⚠️ 위 수치는 화각 70° 가정에 기반한 **계산값**입니다. Phase 0-A에서 `FormatInspector`가 수집한 실측 시야각으로 보정합니다.
>
...
| 노출 | 이동 거리 | 화면 번짐 | 공 지름 대비 | 판정 |
|---|---|---|---|---|
| 1/60 s | 46.3 cm | 106 px | 691 % | ❌ 공(15px)이 완전히 뭉개짐 |
| 1/120 s | 23.1 cm | 53 px | 345 % | ❌ |
| 1/250 s | 11.1 cm | 25 px | 166 % | ⚠️ 실내 폴백. 궤적 정확도 저하 |
| 1/500 s | 5.6 cm | 13 px | 83 % | ⚠️ 경계 |
| **1/1000 s** | **2.8 cm** | **6.3 px** | **41 %** | ✅ |

> 번짐 = 공 속도(px/s) × 노출 시간. 6347 px/s 기준입니다. 관계식의 근거는
> [모션 블러와 공 추적](../08-레퍼런스/ml/MotionBlur.md)에 있고, `teni synth`의
> 테스트가 이 표를 고정합니다 — 표와 코드가 갈라지면 테스트가 깨집니다.
...
> 스윙 분석 모드(5.1)와 **같이 쓸 수 없습니다.** 근거: [ADR-0004](../05-결정/adr/ADR-0004-line-call-mode.md)

| 항목 | 스윙 분석 | **라인 판정** |
|---|---|---|
| 각도 | 코트 장축에 수직 (측면) | **판정할 라인의 연장선** |
| 거리 | 6 m | 라인 전체가 프레임에 들어오는 거리 |
| 높이 | 1.1 m (허리) | 라인이 화면을 **가로지르게** |
| 담는 것 | 선수 (프레임 높이 35~45%) | 라인 + 좌우 여유 |
| 캘리브레이션 | 없음 | **4점 수동 탭** (세션 1회) |

### 왜 라인 연장선인가

지면 위 한 점의 측정 정밀도가 **방향에 따라 갈립니다.**
...
**라인 폭은 5 cm입니다.** 5.1의 측면 배치로는 12 m에서 이미 1픽셀이 라인보다 넓습니다. 판정할 라인이 화면에서 **가로로** 보이게 두면 정밀도가 회복됩니다 — 방송 중계의 라인 카메라가 라인 연장선에 놓이는 이유입니다.

프레이밍도 맞지 않습니다. 5.1의 6 m 배치는 프레임 폭이 8.4 m라 **한쪽 코트의 71%만** 담고, 인/아웃이 갈리는 상대 코트 베이스라인은 프레임 밖입니다.

> ⚠️ 위 수치는 5.3과 마찬가지로 **화각 70° 가정**에 기반한 계산값입니다. [#4](https://github.com/wnsgur9137/TeniTeni/issues/4)의 실측으로 보정합니다. 깊이·가로의 **비율 관계**는 화각과 무관하게 성립합니다.

### 미실측

**정확도를 검증할 방법이 없습니다.** 클레이 코트의 바운스 자국을 자로 재거나 기준 카메라를 따로 두는 수밖에 없습니다. 5.5의 게이트와 같은 성격이며, 그 전까지 판정은 **"명백한 인 / 명백한 아웃 / 판정 불가" 3분류**로만 냅니다.
