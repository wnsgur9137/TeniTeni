...
> **라인 판정 경로는 관찰 대상이 라인 판정일 때만 동작합니다.** "내 스윙"과 카메라 배치가 다르므로 동시에 켜지지 않습니다 — [ADR-0004](../05-결정/adr/ADR-0004-line-call-mode.md).
>
...
├──────────────────────────────────────────────┤
│ TeniVision      Pose / Ball / Court / Swing   │  ← UIKit 의존 없음
│                Render / Metrics              │     영상 입력 → 분석 결과 출력
└──────────────────────────────────────────────┘

격리 도메인 배치는 [동시성과 상태 설계](CONCURRENCY.md)를 따릅니다.
```

**TeniVision을 별도 계층으로 분리하는 이유**가 중요합니다. UI에 의존하지 않으면 고정된 샘플 영상을 입력으로 넣어 **결정론적 회귀 테스트**를 돌릴 수 있습니다. 이 앱에서 가장 가치 있는 테스트가 바로 그것입니다 — "이 영상에서 스윙 12개가 검출되고 평균 팔꿈치 각도가 148°여야 한다"를 CI에서 검증할 수 있습니다.
...
- **분석 수집 최소화**: Firebase Analytics는 화면 전환·기능 사용 여부까지만. 스윙 데이터·영상 메타는 보내지 않는다. 설정에서 끌 수 있게 한다 ([D-13](../05-결정/stack/D-13-observability.md))
