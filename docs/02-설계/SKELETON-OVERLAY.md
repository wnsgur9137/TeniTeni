...
여기 있는 것은 전부 **포즈·렌더** 항목입니다. 궤적 검출 성패와 무관하므로
[0-C 게이트](../04-계획/WORK-PLAN.md)를 기다리지 않고 착수합니다
([작업 순서 9.3의 0-D 표](../04-계획/WORK-PLAN.md)).

| 검증 항목 | 0-D | 실기기 |
|---|---|---|
| `RenderTransform`이 세로/가로, 전면/후면에서 모두 정확한가 | 2 | **불필요** |
| `MTKView` + `CVMetalTextureCache`로 카메라 영상이 60fps로 표시되는가 | 1 | 필요 |
| 스켈레톤이 몸과 어긋나지 않는가 (빠른 스윙에서) | 5 | 필요 |
...
**`RenderTransform`을 맨 앞에 둔 이유**는 실기기 없이 완결되는 유일한 항목이기 때문입니다. 나머지 넷은 카메라·GPU·실제 회전각이 있어야 확인됩니다.

> 마지막 항목의 숫자는 **잠정값**입니다. 실시간 궤적 렌더 패스(0-D 6)가 게이트 이후에 붙으므로, 그때 다시 재야 최종값이 나옵니다.
